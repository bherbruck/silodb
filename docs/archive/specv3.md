...
- **Schema declaration must not depend on any Parquet file existing.**
  Don't derive vtab columns by reading the first file found in the
  directory — that makes `CREATE VIRTUAL TABLE` fail on an empty directory
  (a brand-new device with nothing compacted yet) and creates an ordering
  dependency ("compact once before creating the vtab") that shouldn't
  exist. Columns must be declared explicitly via vtab arguments (extend
  the `silodb('dir/', table=name, ts_column=name)` convention with a
  columns/schema argument), sourced from the hot table's already-known
  schema. Vtab creation must be a pure function of its arguments — zero
  file I/O at `xConnect` time, works identically whether the directory has
  zero files or a thousand.
- **`dir` is a shared base directory across all tables, `table` is
  required.** (Supersedes an earlier decision in this file that treated
  `dir` as the per-table directory itself — that was correct for the
  design at the time, this replaces it deliberately, not by drift.) The
  actual per-table directory is `dir.join(table)`, computed inside
  `silodb`'s connect logic. This is not the implicit-magic pattern rejected
  earlier in design (a bare name resolving via hidden config/env lookup) —
  both `dir` and `table` are explicit arguments visible directly in the
  `CREATE VIRTUAL TABLE` statement; nothing about the resulting path
  requires consulting anything outside the SQL itself. `table` can no
  longer default to the directory's basename (that only made sense when
  `dir` meant the per-table directory) — it must be passed explicitly.
  Directory creation is `compact_bucket()`'s responsibility (idempotent
  `mkdir -p` on `dir.join(table)` before its first write for that table),
  not `xConnect`'s — a read-side connect should not have a directory-
  creation side effect, and must not fail if the directory doesn't exist
  yet, consistent with the empty-catalog fix above.
- **Recommended on-disk layout** (a convention for the embedding
  ...
  time.
- **Late-arriving rows for an already-compacted bucket must not be a hard
  error.** Compact them into a new, separate file with the
  same/overlapping range, registered as a separate catalog row — do not
  merge into the sealed original file (preserves the "never rewrite a
  written file" invariant) and do not silently drop the rows. The catalog
  and pruning query already support multiple files with overlapping ranges
  for one logical table; this is not a new read-side capability, it's using
  the existing one for a case the write side wasn't yet handling.
