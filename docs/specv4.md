...
These came up during Phase 0–3 implementation and are settled here so
they're not re-litigated or silently reinterpreted in a later session.
Where noted, the actual implementation improved on an earlier draft of
this resolution — the doc reflects what was built, not the first idea.

- **Schema declaration borrows the hot table's live schema — no explicit
  columns argument, no dependency on any Parquet file existing.**
  (Supersedes an earlier draft of this resolution that proposed an explicit
  columns argument — the implementation found a cleaner mechanism.)
  `xConnect` receives the connection it's being created in; since the hot
  table is guaranteed to already exist (it's the app's own OLTP write
  target, created at startup before any vtab or compaction), the vtab
  introspects that table's schema directly and runs it through
  `bucket_arrow_schema` in `silodb-schema` — the same conversion function
  `compact_bucket` uses when writing Parquet files. One schema source, read
  live by both paths; they cannot disagree. Vtab creation needs zero files
  and zero catalog rows — works identically on a brand-new device with
  nothing compacted yet.
- **`dir` is a shared base directory across all tables; the vtab's own
  declared name is the default logical table.** (Supersedes an earlier
  draft that treated `dir` as the per-table directory itself, and a later
  draft that made `table=` a required argument — the implementation found
  a way to avoid that redundancy.) `CREATE VIRTUAL TABLE sensor_a USING
  silodb('buckets/')` — the actual per-table directory is
  `buckets/sensor_a/`, derived from the name SQLite already assigns the
  vtab at creation (standard vtab API), not a separately repeated
  argument. `table=`, `ts_column=`, `hot_table=` are available as explicit
  overrides for cases where those need to differ from the defaults. This
  is not the implicit-magic pattern rejected earlier in design (a bare
  name resolving via hidden config/env lookup) — every input to the
  resulting path is visible directly in the `CREATE VIRTUAL TABLE`
  statement itself. Directory creation is `compact_bucket`'s
  responsibility (idempotent, on first write for that table), not
  `xConnect`'s.
- **Recommended on-disk layout** (a convention for the embedding
  application, not enforced by the crate itself): one base cold-storage
  directory sitting beside the hot `.sqlite` file, e.g. `cold/` next to
  `hot.sqlite`, with per-table subdirectories created automatically
  underneath it as tables are compacted for the first time. This keeps
  exactly one new thing at the top level next to the database, not one
  directory per table accumulating there as sensors/streams are added over
  time.
- **Late-arriving rows for an already-compacted bucket produce a follow-up
  file, not an error.** `compact_bucket` names files itself —
  `bucket-<start>-<end>-<seq>.parquet`. Re-running after a successful
  compaction is a no-op; re-running after a crash recomputes the same
  `seq` and rewrites a byte-identical file before the transaction commits
  (see the compaction-cost invariant above — this doesn't touch prior
  buckets). Rows that land in an already-closed bucket's range get a new
  `seq`, written as a separate file and a separate catalog row with an
  overlapping range — the catalog and pruning query already support
  multiple files per range, this uses that capability rather than adding a
  new one. Genuine errors are kept genuine: a catalog row with no matching
  file on disk, or malformed data (wrong SQLite storage class for a
  declared column), still fail loudly rather than being absorbed.
- **Vtab trust flag: neither `DirectOnly` nor `Innocuous` — default trust
  level.** `DirectOnly` would forbid use inside views, which breaks the
  hot/cold union-view pattern outright. `Innocuous` was the first instinct
  but doesn't hold up on a closer read of what it actually asserts: it
  claims a module can't leak information or affect state beyond its return
  value, and a module that opens caller-supplied file paths doesn't
  strictly meet that regardless of how trusted this particular deployment
  is. Leaving the default (relying on SQLite's trusted-schema mode,
  appropriate here since the schema is entirely self-authored, not
  externally-supplied SQL) gets the same practical outcome — works fine in
  views — without asserting something that isn't literally true about the
  module.
